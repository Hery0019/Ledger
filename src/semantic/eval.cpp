...
// Everything below threads `subs` down to nested eval() calls.
class Evaluator {
public:
    Evaluator(const Row& row, const SubqueryRows* subs) noexcept : row_(row), subs_(subs) {}

    Result<Value> operator()(const BoundExpr& expr) {
        return std::visit(
            [&](const auto& n) -> Result<Value> {
                ...
                if constexpr (std::is_same_v<N, Value>) {
                    ...
                    return inSubquery(n);
                } else if constexpr (std::is_same_v<N, BoundExists>) {
                    LEDGER_TRY(rows, subqueryRows(n.slot));
                    return Value::boolean(rows->empty() == n.negated);
                } else if constexpr (std::is_same_v<N, BoundScalarSubquery>) {
                    LEDGER_TRY(rows, subqueryRows(n.slot));
                    if (rows->empty()) return Value::null();
                    if (rows->size() > 1) {
                        return makeError(ErrorCode::TypeError,
                                         "scalar subquery returned " + std::to_string(rows->size()) +
                                             " rows; at most one is allowed");
                    }
                    return (*rows)[0][0];
                } else {
                    return cast(n);
...
private:
    Result<const std::vector<Row>*> subqueryRows(std::size_t slot) const {
        if (!subs_ || slot >= subs_->size()) {
            return makeError(ErrorCode::Internal, "subquery evaluated without its rows");
        }
        return &(*subs_)[slot];
    }

    Result<Value> unary(const BoundUnary& u) {
        LEDGER_TRY(v, (*this)(*u.operand));
    ...
    Result<Value> logical(BinaryOp op, const BoundBinary& b) {
        LEDGER_TRY(l, (*this)(*b.lhs));
        LEDGER_TRY(r, (*this)(*b.rhs));
        // Three-valued logic. Both sides are evaluated: no short-circuit, so
        // that a data error (division by zero) does not depend on operand
        // order.
    ...
    Result<Value> binary(const BoundBinary& b) {
        if (b.op == BinaryOp::And || b.op == BinaryOp::Or) return logical(b.op, b);
    ...
    Result<Value> inList(const BoundInList& in) {
        LEDGER_TRY(v, (*this)(*in.value));
        if (v.isNull()) return Value::null();
        bool sawNull = false;
        for (const auto& item : in.items) {
            LEDGER_TRY(x, (*this)(*item));
            if (x.isNull()) {
                sawNull = true;
                continue;
            }
    ...
    // Same rules as inList, over the single column of the subquery's rows.
    Result<Value> inSubquery(const BoundInSubquery& in) {
        LEDGER_TRY(v, (*this)(*in.value));
        if (v.isNull()) return Value::null();
        LEDGER_TRY(rows, subqueryRows(in.slot));
        bool sawNull = false;
    ...
    Result<Value> like(const BoundLike& l) {
        LEDGER_TRY(v, (*this)(*l.value));
        LEDGER_TRY(p, (*this)(*l.pattern));
        if (v.isNull() || p.isNull()) return Value::null();
        return Value::boolean(likeMatch(v.asText(), p.asText()) != l.negated);
    }

    Result<Value> call(const BoundCall& c) {
        ...
        // COALESCE / NULLIF have their own NULL rules; every other function
        // is NULL as soon as an argument is.
        switch (c.func) {
        ...
        switch (c.func) {
            ...
            case ScalarFunc::Round: {
                ...
                if (digits < -18 || digits > 18) {
                    return makeError(ErrorCode::TypeError, "round(): digits out of range");
                }
                double scale = 1.0;
    ...
    Result<Value> caseExpr(const BoundCase& c) {
        for (const auto& [cond, result] : c.whens) {
            LEDGER_TRY(v, (*this)(*cond));
            if (!v.isNull() && v.asBool()) return (*this)(*result);
        }
        if (c.elseExpr) return (*this)(*c.elseExpr);
        return Value::null();
    }

    Result<Value> cast(const BoundCast& c) {
        LEDGER_TRY(v, (*this)(*c.operand));
...
Result<Value> eval(const BoundExpr& expr, const Row& row, const SubqueryRows* subs) {
    return Evaluator{row, subs}(expr);
}
...
bool isConstant(const BoundExpr& expr) noexcept {
    return std::visit(
        [](const auto& n) -> bool {
            ...
            if constexpr (std::is_same_v<N, Value>) {
                return true;
            } else if constexpr (std::is_same_v<N, BoundColumn> || std::is_same_v<N, BoundInSubquery> ||
                                 std::is_same_v<N, BoundExists> || std::is_same_v<N, BoundScalarSubquery>) {
                return false;
            } else if constexpr (std::is_same_v<N, BoundBinary>) {
