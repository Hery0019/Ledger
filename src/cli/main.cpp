...
bool dotCommand(const Console& con, Database& db, const std::string& line, bool& quit) {
    ...
    if (cmd == ".quit" || cmd == ".exit") {
        ...
        const auto indexes = db.catalog().indexes();
        if (indexes.empty()) std::cout << con.paint(ansi::dim, "no user indexes (CREATE INDEX name ON table (column) to add one)") << '\n';
        for (const auto& d : indexes) {
            std::cout << d.name << con.paint(ansi::dim, " on ") << d.table << '(' << d.column << ")\n";
        }
    } else if (cmd == ".schema") {
        if (arg.empty()) {
        ...
        item(".indexes", "list user indexes");
